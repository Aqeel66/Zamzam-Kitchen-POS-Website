...
class ExtraManager extends StatefulWidget {
  ...
  final Function() onExtrasChanged;
...
class _ExtraManagerState extends State<ExtraManager> {
  ...
  Future<void> _addExtra() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://zamzamkitchen.net/api/menu/extras'),
      ...
      if (response.statusCode == 201) {
        ...
        _showError('Failed to add extra');
      }
    } catch (e) {
  ...
  Future<void> _deleteExtra(dynamic extra) async {
    setState(() => _isLoading = true);
    try {
      final response = await http.delete(
        Uri.parse('http://zamzamkitchen.net/api/menu/extras/${extra['id']}'),
      );

      if (response.statusCode == 200) {
        widget.onExtrasChanged();
      } else {
        _showError('Failed to delete extra');
      }
    } catch (e) {
      _showError(e.toString());
    } finally {
  ...
  Widget build(BuildContext context) {
    final extras = widget.menuItem['extras'] as List<dynamic>? ?? [];
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      ElevatedButton(
                        onPressed: _isLoading ? null : _addExtra,
