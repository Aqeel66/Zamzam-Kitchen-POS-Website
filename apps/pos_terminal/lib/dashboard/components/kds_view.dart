...
class KDSView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                LocalizationService().translate('no_kds_orders'),
                style: TextStyle(
                  color: themeHint,
                  fontSize: 18,
    ...
    int preparingCount = placedOrders
        .where(
          (o) => (o['status']?.toString() ?? '').toLowerCase() == 'preparing',
        )
        .length;
    int readyCount = placedOrders
        .where((o) => (o['status']?.toString() ?? '').toLowerCase() == 'ready')
        .length;
    int rejectedCount = placedOrders
        .where(
          (o) => (o['status']?.toString() ?? '').toLowerCase() == 'rejected',
        )
        .length;
              ...
              child: Row(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      Text(
                        LocalizationService().translate('kitchen_production'),
                        style: TextStyle(
                          color: themeText,
                          fontSize: 22,
                      ...
                      Text(
                        '${kdsOrders.length} ${LocalizationService().translate('active_tickets')}',
                  ...
                  Container(
                    ...
                    child: TabBar(
                      ...
                      tabs: [
                        ...
                        _buildKDSMainTab(
                          LocalizationService().translate('preparing'),
                          preparingCount,
                        ),
                        _buildKDSMainTab(
                        ...
                        _buildKDSMainTab(
                          LocalizationService().translate('rejected'),
                          rejectedCount,
                        ),
                      ],
                    ),
  ...
  Widget _buildKDSTab(String status) {
    ...
    if (filteredOrders.isEmpty) {
      return Center(
        child: Text(
          '${LocalizationService().translate('no_orders_in')} ${LocalizationService().translate(status.toLowerCase())}',
