...
class _ReservationsViewState extends State<ReservationsView> {
  ...
  void _initControllers() {
    ...
    _partySizeController = TextEditingController(
      text: res?['party_size']?.toString() ?? '',
    );
    _dateController = TextEditingController(
      text: res?['reservation_date']?.toString().split('T')[0] ?? '',
    );
    _timeController = TextEditingController(
      text: res?['reservation_time']?.toString().substring(0, 5) ?? '',
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    filtered.sort((a, b) {
      ...
      final db =
          ...
          DateTime(2000);
      return widget.orderSortDirection == 'Ascending'
          ? da.compareTo(db)
          : db.compareTo(da);
    });
                  ...
                  child: Row(
                    children: [
                      ...
                      ElevatedButton.icon(
                        ...
                        label: Text(
                          LocalizationService().translate('add_new_booking'),
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final bool isSelected =
                                widget.selectedReservation?['id'] == res['id'];
  ...
  String _getTableLabel(Map<String, dynamic> res) {
    if (res['assigned_table_number'] != null &&
        res['assigned_table_number'].toString().isNotEmpty) {
      return res['assigned_table_number'].toString();
    }
    if (res['table_number'] != null &&
        res['table_number'].toString().isNotEmpty) {
  ...
  bool _hasTableAssigned(Map<String, dynamic> res) {
    return (res['assigned_table_number'] != null &&
            res['assigned_table_number'].toString().isNotEmpty) ||
        (res['table_number'] != null &&
            res['table_number'].toString().isNotEmpty) ||
          ...
          children: [
              ...
              child: Row(
                children: [
                  _buildInfoItem(
                    ...
                    (res['reservation_date']?.toString() ?? '').split('T')[0],
                  ),
                  const SizedBox(width: 16),
                  _buildInfoItem(
                    Icons.access_time_rounded,
                    (res['reservation_time']?.toString() ?? '').substring(0, 5),
                  ),
                ],
              ),
            ),
            ...
            Row(
              ...
              children: [
                OriginBadge(
                  origin: res['origin'],
                  themePrimary: widget.themePrimary,
                ),
                const SizedBox(width: 12),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSectionHeader(
            'CONTACT INFORMATION',
            onEdit: _isEditing ? null : () => setState(() => _isEditing = true),
          ),
            ...
            Row(
              children: [
                ...
                TextButton(
                  ...
                  child: Text(
                    'Cancel',
                    style: TextStyle(color: widget.themeHint, fontSize: 13),
                  ),
                ),
              ],
            ),
                ...
                onPressed: res['table_id'] != null
                    ...
                    : () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Please assign a table first'),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    onPressed: () =>
                        widget.onUpdateStatus(res['id'], 'No-Show'),
                ...
                Expanded(
                  child: TextButton(
                    onPressed: () =>
                        widget.onUpdateStatus(res['id'], 'Cancelled'),
                    child: const Text(
                      'CANCEL',
                      style: TextStyle(
                        color: Colors.red,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'GUEST SEATED AT TABLE ${res['table_number']}',
                      style: const TextStyle(
                        color: Colors.green,
                        fontWeight: FontWeight.bold,
                        fontSize: 14,
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: widget.themeBorder),
            ),
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: widget.themePrimary),
            ),
          ),
        ),
      ],
        ...
        child: DropdownButton<String>(
          ...
          items: tables.map((t) {
            return DropdownMenuItem<String>(
              value: t.id,
              child: Text(
                'Table ${t.label} (Avail: ${t.capacity - t.currentOccupancy} / ${t.capacity})',
  ...
  Widget _buildStatusBadge(String status) {
    ...
    return BaseBadge(label: status, color: color);
  }
  ...
  Widget _buildReservationFilters() {
    ...
    final Map<String, int> counts = {
      ...
      'Upcoming': widget.reservations
          .where(
            (res) => res['status'] == 'Confirmed' || res['status'] == 'Pending',
          )
          .length,
      'Pending': widget.reservations
          .where((res) => res['status'] == 'Pending')
          .length,
      'Confirmed': widget.reservations
          .where((res) => res['status'] == 'Confirmed')
          .length,
      'Seated': widget.reservations
          .where((res) => res['status'] == 'Seated')
          .length,
    };
        ...
        children: filters.map((f) {
          ...
          return Padding(
            ...
            child: ChoiceChip(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
                      color: sel ? Colors.white : widget.themeHint,
                    ),
                  ),
                  if (count > 0 || f == 'ALL') ...[
                    const SizedBox(width: 8),
              ...
              side: BorderSide(
                color: sel ? widget.themePrimary : widget.themeBorder,
              ),
              showCheckmark: false,
