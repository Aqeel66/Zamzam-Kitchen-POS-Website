      ...
      <aside className="w-[400px] bg-white border-r border-slate-200 flex flex-col overflow-hidden shadow-inner">
         <div className="p-8 pb-6">
            <div className="flex items-center justify-between mb-6">
               <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">{viewMode === 'products' ? 'Product' : 'Category'} <span className="text-zamzam-teal">Vault</span></h2>
            ...
            <div className="relative group">
               ...
               <input type="text" placeholder={`Search ${viewMode}...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-6 text-xs font-black uppercase outline-none focus:border-zamzam-teal shadow-inner" />
            </div>
         </div>
              ...
              filteredProducts.map((item) => (
                <div key={item.id} onClick={() => handleSelectItem(item)} className={cn("p-5 rounded-[2rem] border transition-all cursor-pointer flex items-center gap-5 group", selectedItem?.id === item.id ? "bg-slate-900 border-slate-900 shadow-2xl scale-[1.02]" : "bg-white border-transparent hover:border-slate-100")}>
                   ...
                   <div className="flex-1 min-w-0">
                      <p className={cn("text-[9px] font-black uppercase tracking-widest mb-1", selectedItem?.id === item.id ? "text-zamzam-yellow" : "text-zamzam-teal")}>{item.categoryName}</p>
                      <h3 className={cn("text-xs font-black uppercase truncate", selectedItem?.id === item.id ? "text-white" : "text-slate-900")}>{item.name}</h3>
              ...
              filteredCategories.map((cat) => (
                <div key={cat.id} onClick={() => handleSelectCategory(cat)} className={cn("p-6 rounded-[2.5rem] border transition-all cursor-pointer flex items-center justify-between group", selectedCategory?.id === cat.id ? "bg-slate-900 border-slate-900 shadow-2xl scale-[1.02]" : "bg-white border-transparent hover:border-slate-100")}>
                   <div className="flex items-center gap-5">
                      <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black shadow-lg", selectedCategory?.id === cat.id ? "bg-zamzam-teal text-white" : "bg-slate-50 text-slate-900")}>{(cat.title || cat.name).charAt(0)}</div>
                      <div>
                         <h3 className={cn("text-[11px] font-black uppercase tracking-widest", selectedCategory?.id === cat.id ? "text-white" : "text-slate-900")}>{cat.title || cat.name}</h3>
          ...
          {viewMode === 'products' && selectedItem ? (
            <motion.div key={selectedItem.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-12 max-w-[1400px] mx-auto space-y-10">
               ...
               <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-4xl font-black text-slate-900 uppercase tracking-tighter mb-2">{selectedItem.name}</h1>
                    <div className="flex items-center gap-3">
                       <span className="px-3 py-1 bg-slate-900 text-white text-[9px] font-black uppercase rounded-lg tracking-widest">{selectedItem.categoryName || 'Uncategorized'}</span>
                       <span className="text-slate-300">/</span>
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Product Mission Control</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                     <div className="text-right">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Selling Price</p>
                        <p className="text-4xl font-black text-zamzam-teal">{selectedItem.price} <span className="text-sm font-bold">{currency}</span></p>
                     </div>
                     <button onClick={saveProduct} disabled={isSubmitting} className="bg-slate-900 text-white font-black px-12 py-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center gap-2 hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-50"><Save size={24} /><span className="text-[10px] uppercase tracking-[0.2em]">{isSubmitting ? 'Saving...' : 'Deploy'}</span></button>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-12 gap-10">
                  ...
                  <div className="col-span-12 lg:col-span-5 space-y-8">
                     <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl space-y-8">
                        <div className="flex items-center gap-4 text-slate-900"><div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center"><Edit2 size={20} /></div><h3 className="text-lg font-black uppercase">Core Details</h3></div>
                        <div className="space-y-6">
                           <div className="space-y-2">
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Product Name</p>
                              <input value={selectedItem.name} onChange={(e) => setSelectedItem((p: any) => ({ ...p, name: e.target.value }))} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-black uppercase outline-none focus:border-zamzam-teal transition-all" />
                           </div>
                           <div className="grid grid-cols-2 gap-6">
                              <div className="space-y-2">
                                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Price ({currency})</p>
                                 <input type="number" value={selectedItem.price} onChange={(e) => setSelectedItem((p: any) => ({ ...p, price: parseFloat(e.target.value) }))} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-black outline-none focus:border-zamzam-teal transition-all" />
                              </div>
                              <div className="space-y-2">
                                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Category</p>
                                 <select value={selectedItem.category_id} onChange={(e) => setSelectedItem((p: any) => ({ ...p, category_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-[11px] font-black uppercase outline-none focus:border-zamzam-teal appearance-none cursor-pointer">
                           ...
                           <div className="grid grid-cols-2 gap-6">
                              <div className="space-y-2">
                                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Prep Station</p>
                                 <select value={selectedItem.prep_station} onChange={(e) => setSelectedItem((p: any) => ({ ...p, prep_station: e.target.value }))} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-[11px] font-black uppercase outline-none focus:border-zamzam-teal appearance-none cursor-pointer">
                              ...
                              <div className="flex items-center justify-between pt-6">
                                 <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-black uppercase">{selectedItem.in_stock ? 'In Stock' : 'Out of Stock'}</span>
                                 </div>
                  ...
                  <div className="col-span-12 lg:col-span-7 space-y-8">
                     <div className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-10"><div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-zamzam-yellow shadow-xl"><FlaskConical size={24} strokeWidth={2.5} /></div><div><h3 className="text-xl font-black uppercase">Recipe Engine</h3><p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Inventory Linking & Costing</p></div></div>
                        ...
                        <div className="space-y-4 mb-10">
                           {selectedItem.recipe.map((r: any, i: number) => (
                             <div key={i} className="flex items-center justify-between bg-white/5 p-5 rounded-2xl border border-white/5 group hover:bg-white/10 transition-all">
                                <div><p className="text-xs font-black uppercase tracking-widest text-zamzam-yellow">{r.ingredient_name}</p><p className="text-[9px] font-bold text-white/40 uppercase mt-1">Cost contribution: {(r.quantity * (r.cost_price || 0)).toFixed(2)} {currency}</p></div>
                                <div className="flex items-center gap-6"><span className="text-xs font-black">{r.display_qty || r.quantity} {r.display_unit || r.unit}</span><button onClick={() => setSelectedItem((p: any) => ({ ...p, recipe: p.recipe.filter((_: any, idx: number) => idx !== i) }))} className="text-white/20 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button></div>
                             </div>
                           ))}
                        </div>
                        ...
                        <div className="grid grid-cols-12 gap-4" ref={dropdownRef}>
                           <div className="col-span-12 relative">
                              <button onClick={() => setIsIngSearchOpen(!isIngSearchOpen)} className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-6 py-5 text-left flex items-center justify-between hover:bg-slate-700 transition-all shadow-inner"><div className="flex items-center gap-4"><Search size={20} className="text-zamzam-yellow" /><p className="text-sm font-black text-white/40 uppercase">{selectedIngredient ? selectedIngredient.name : 'Find Inventory Item...'}</p></div><ChevronDown className={cn("text-zamzam-yellow transition-transform", isIngSearchOpen ? "rotate-180" : "0")} /></button>
                              <AnimatePresence>{isIngSearchOpen && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 right-0 top-full mt-4 bg-white rounded-[2rem] shadow-[0_30px_90px_rgba(0,0,0,0.6)] z-[100] overflow-hidden"><div className="p-4 border-b border-slate-100 bg-slate-50"><input autoFocus placeholder="SEARCH..." value={ingSearchQuery} onChange={(e) => setIngSearchQuery(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-xs font-black uppercase outline-none text-slate-900" /></div><div className="max-h-[250px] overflow-y-auto no-scrollbar p-2">{filteredInventory.map(i => (<button key={i.id} onClick={() => { setRecipeDraft(p => ({ ...p, inventory_id: i.id.toString(), unit: i.unit })); setIsIngSearchOpen(false); }} className="w-full text-left px-6 py-4 rounded-xl flex items-center justify-between hover:bg-slate-50 transition-all mb-1"><div><p className="text-xs font-black uppercase text-slate-900">{i.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Unit: {i.unit}</p></div><ChevronRight size={14} className="text-slate-200" /></button>))}</div></motion.div>
                              )}</AnimatePresence>
                           </div>
                           <div className="col-span-12 grid grid-cols-12 gap-4 mt-2">
                              <input type="number" placeholder="QTY" value={recipeDraft.quantity} onChange={(e) => setRecipeDraft(p => ({ ...p, quantity: e.target.value }))} className="col-span-5 bg-slate-800 border border-slate-700 rounded-2xl px-6 py-5 text-sm font-black text-white outline-none focus:border-zamzam-yellow transition-all" />
                              <select value={recipeDraft.unit} onChange={(e) => setRecipeDraft(p => ({ ...p, unit: e.target.value }))} className="col-span-4 bg-slate-800 border border-slate-700 rounded-2xl px-5 py-5 text-[11px] font-black text-zamzam-yellow uppercase outline-none cursor-pointer">
                              ...
                              <button onClick={commitRecipeDraft} className="col-span-3 bg-zamzam-yellow text-slate-900 rounded-2xl font-black text-sm shadow-xl shadow-yellow-500/20 active:scale-95 transition-all"><Plus size={24} strokeWidth={3} className="mx-auto" /></button>
                           </div>
                        </div>

                        <div className="grid grid-cols-3 gap-8 mt-12 pt-12 border-t border-white/10">
                           <div className="space-y-1"><p className="text-[9px] font-black text-white/30 uppercase tracking-widest">Plate Cost</p><p className="text-2xl font-black text-white">{financials.totalCost.toFixed(2)} <span className="text-[10px] text-white/40">{currency}</span></p></div>
                           <div className="space-y-1 text-center"><p className="text-[9px] font-black text-white/30 uppercase tracking-widest">Food Cost %</p><p className={cn("text-2xl font-black", financials.fcPercent > 35 ? "text-red-400" : "text-zamzam-teal")}>{financials.fcPercent.toFixed(1)}%</p></div>
                           <div className="space-y-1 text-right"><p className="text-[9px] font-black text-white/30 uppercase tracking-widest">Gross Profit</p><p className="text-2xl font-black text-zamzam-yellow">{financials.gp.toFixed(2)} {currency}</p></div>
                        </div>
                     </div>

                     <div className="grid grid-cols-2 gap-8">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-lg space-y-6">
                           <div className="flex items-center gap-3 text-slate-900"><Maximize2 size={20} /><h4 className="text-xs font-black uppercase">Variants</h4></div>
                           <div className="space-y-2 max-h-[150px] overflow-y-auto no-scrollbar">
                              {selectedItem.variants.map((v: any, i: number) => (
                                <div key={i} className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100 group"><span className="text-[10px] font-black uppercase text-slate-900">{v.name}</span><div className="flex items-center gap-4"><span className="text-[10px] font-bold text-zamzam-teal">+{v.price_offset} {currency}</span><button onClick={() => setSelectedItem((p: any) => ({ ...p, variants: p.variants.filter((_: any, idx: number) => idx !== i) }))} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={12} /></button></div></div>
                              ))}
                           </div>
                           <div className="flex gap-2">
                              <input placeholder="SIZE/TYPE..." value={variantDraft.name} onChange={(e) => setVariantDraft(p => ({ ...p, name: e.target.value }))} className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-[10px] font-black uppercase outline-none focus:border-zamzam-teal transition-all" />
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-lg space-y-6">
                           <div className="flex items-center gap-3 text-slate-900"><Zap size={20} /><h4 className="text-xs font-black uppercase">Add-ons</h4></div>
                           <div className="space-y-2 max-h-[150px] overflow-y-auto no-scrollbar">
                           ...
                           <div className="flex gap-2">
                              <input placeholder="EXTRA..." value={extraDraft.name} onChange={(e) => setExtraDraft(p => ({ ...p, name: e.target.value }))} className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-[10px] font-black uppercase outline-none focus:border-zamzam-teal transition-all" />
            ...
            <motion.div key={selectedCategory.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-12 max-w-[1000px] mx-auto space-y-12">
               <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-5xl font-black text-slate-900 uppercase tracking-tighter mb-2">{selectedCategory.title || selectedCategory.name}</h1>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Category Management Hub</p>
                  </div>
                  <button onClick={saveCategory} disabled={isSubmitting} className="bg-zamzam-teal text-white font-black px-12 py-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center gap-2 hover:bg-teal-400 transition-all active:scale-95 disabled:opacity-50"><Save size={24} /><span className="text-[10px] uppercase tracking-[0.2em]">{isSubmitting ? 'Syncing...' : 'Save Category'}</span></button>
               </div>
                  ...
                  <div className="grid grid-cols-2 gap-10">
                     <div className="space-y-8">
                        <div className="space-y-3">
                           <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">Display Title</p>
                           <input value={selectedCategory.title || selectedCategory.name} onChange={(e) => setSelectedCategory((p: any) => ({ ...p, title: e.target.value, name: e.target.value }))} className="w-full bg-slate-50 border border-slate-100 rounded-[1.5rem] p-6 text-lg font-black uppercase outline-none focus:border-zamzam-teal transition-all shadow-inner" />
                        </div>
                        <div className="space-y-3">
                           <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">Description (Web & App)</p>
                     ...
                     <div className="space-y-8">
                        <div className="space-y-3">
                           <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">Category Visual</p>
                           <div className="w-full aspect-video bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 text-slate-300 hover:text-zamzam-teal hover:border-zamzam-teal transition-all cursor-pointer overflow-hidden group">
                              {selectedCategory.image ? (
                                ...
                                <><ImageIcon size={48} strokeWidth={1.5} /><span className="text-[10px] font-black uppercase tracking-widest">Upload Cover Image</span></>
                              )}
                           </div>
                        </div>
                        <div className="p-8 bg-slate-900 rounded-[2rem] text-white">
                           <div className="flex items-center gap-4 mb-4"><div className="w-10 h-10 bg-zamzam-teal rounded-xl flex items-center justify-center text-white"><Package size={20} /></div><h4 className="text-xs font-black uppercase tracking-widest">Inventory Load</h4></div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-200">
               ...
               <h2 className="text-2xl font-black uppercase tracking-[0.6em] mt-12 text-slate-300">Design Hub</h2>
            </div>
          )}
        </AnimatePresence>
      </main>
