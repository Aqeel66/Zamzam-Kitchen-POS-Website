        ...
        <div className="flex items-end justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
               ...
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Procurement Pipeline</span>
            </div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Purchase <span className="text-indigo-500">Orders</span></h1>
          </div>
          
          <button 
            onClick={() => setIsCreateModalOpen(true)}
            className="bg-indigo-500 px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl shadow-indigo-500/20 hover:scale-105 transition-transform flex items-center gap-3"
          >
            <Plus size={18} />
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
              ...
              <tbody className="divide-y divide-slate-50">
                 {filteredOrders.map((order) => (
                    <tr key={order.id} className="group hover:bg-slate-50/30 transition-colors">
                       ...
                       <td className="px-10 py-7 text-sm font-black text-slate-500 uppercase tracking-widest">{order.invoice_number || '---'}</td>
                       <td className="px-10 py-7">
                          <p className="text-sm font-black text-slate-900">{order.supplier_name}</p>
                       </td>
                       <td className="px-10 py-7">
                          <div className="flex -space-x-2">
                             {(order.items || []).slice(0, 3).map((item, i) => (
                                <div key={i} className="w-8 h-8 rounded-full bg-white border-2 border-slate-50 flex items-center justify-center shadow-sm" title={item.item_name}>
                       ...
                       <td className="px-10 py-7">
                          <p className="text-lg font-black text-slate-900 tabular-nums">{parseFloat(order.total_amount).toLocaleString()} <span className="text-[10px] text-slate-300 font-bold uppercase">{currency}</span></p>
                       </td>
                       <td className="px-10 py-7">
                          <StatusBadge status={order.status} />
                       </td>
                       <td className="px-10 py-7 text-right flex items-center justify-end gap-2">
                          <button 
                            onClick={() => setSelectedOrder(order)}
                            className="px-6 py-3 bg-slate-50 text-[10px] font-black uppercase tracking-widest text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm"
                          >
                             Manage Order
                          </button>
                          <button 
           ...
           {filteredOrders.length === 0 && (
              <div className="p-20 text-center">
                 ...
                 <p className="text-sm font-black text-slate-300 uppercase tracking-widest">No matching purchase orders found</p>
              </div>
           )}
        </div>
      </div>
...
const StatusBadge = ({ status }: { status: string }) => {
   ...
   return (
      <span className={cn("px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border shadow-sm", styles[status as keyof typeof styles])}>
         {status}
      </span>
   );
};
         ...
         <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-5xl h-[85vh] rounded-[3rem] shadow-2xl flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col p-12 border-r border-slate-50">
               <div className="flex items-center justify-between mb-10">
                  <div>
                     <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Create New <span className="text-indigo-500">PO</span></h2>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Drafting a new supplier request</p>
                  </div>
               ...
               <div className="space-y-8 flex-1 overflow-y-auto no-scrollbar pr-4">
                  ...
                  <div className="space-y-4">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Order Items</label>
                     <div className="space-y-3">
                        {orderItems.map((item, idx) => (
                           <div key={idx} className="bg-slate-50/50 rounded-2xl p-4 flex items-center gap-4 group">
                              <div className="flex-1">
                                 <p className="text-xs font-black text-slate-900 uppercase">{item.item_name}</p>
                              ...
                              <div className="w-24">
                                 <input 
                                    ...
                                    className="w-full bg-white border-none rounded-xl p-2.5 text-center text-xs font-black outline-none shadow-sm"
                                 />
                              </div>
                              <div className="w-24 relative">
                                 ...
                                 <input 
                                    ...
                                    className="w-full bg-white border-none rounded-xl p-2.5 pl-6 text-xs font-black outline-none shadow-sm"
                                 />
                              </div>
                        ...
                        {orderItems.length === 0 && (
                           <div className="py-12 border-2 border-dashed border-slate-100 rounded-3xl text-center">
                              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Select items from the right to add to order</p>
                           </div>
                        )}
                     </div>
                  </div>
               ...
               <div className="mt-10 pt-8 border-t border-slate-50 flex items-center justify-between">
                  <div>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Grand Total ({currency})</p>
                     <p className="text-4xl font-black text-slate-900 tracking-tighter tabular-nums">{total.toFixed(2)}</p>
                  </div>
                  <button 
                     onClick={handleSave}
                     disabled={isSaving || !supplierId || orderItems.length === 0}
                     className="px-12 py-5 bg-indigo-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 transition-all flex items-center gap-3"
                  >
            ...
            <div className="w-96 bg-slate-50 p-10 flex flex-col gap-6">
               <div className="relative">
                  ...
                  <input 
                     ...
                     className="w-full bg-white border-none rounded-xl py-3.5 pl-12 pr-4 text-xs font-black outline-none shadow-sm shadow-slate-200/50"
                  />
               </div>

               <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-2">Inventory Stock</p>
                  {inventory
                     ...
                     .map((item: InventoryItem) => (
                        ...
                        >
                           <div>
                              <p className="text-xs font-black text-slate-900 uppercase group-hover:text-indigo-600 transition-colors">{item.name}</p>
                              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{item.unit}</p>
                           </div>
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden p-12">
         <div className="flex items-start justify-between mb-10">
            <div>
               <div className="flex items-center gap-3 mb-2">
                  <StatusBadge status={order.status} />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Order ID: #{order.id}</span>
               </div>
               <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">{order.supplier_name}</h2>
         ...
         <div className="space-y-6 max-h-[40vh] overflow-y-auto no-scrollbar pr-4">
            ...
            {order.items?.map((item: any, idx: number) => (
               <div key={idx} className="flex items-center justify-between py-4 border-b border-slate-50 last:border-0">
                  <div>
                     <p className="text-sm font-black text-slate-900 uppercase tracking-tight">{item.item_name}</p>
                     <p className="text-[10px] font-bold text-slate-400 mt-0.5">{item.quantity} {item.unit} @ {parseFloat(item.unit_price).toFixed(2)} {currency}</p>
                  </div>
                  <p className="text-base font-black text-slate-900 tabular-nums">{parseFloat(item.subtotal).toFixed(2)} {currency}</p>
               </div>
            ))}
         </div>
         ...
         <div className="mt-10 pt-10 border-t border-slate-100 flex items-center justify-between">
            <div>
               ...
               <p className="text-3xl font-black text-slate-900 tabular-nums">{parseFloat(order.total_amount).toFixed(2)} <span className="text-sm text-slate-300 font-bold uppercase">{currency}</span></p>
            </div>
            ...
            <div className="flex gap-2">
               {order.status === 'Pending' && (
                  <>
                     <button 
                        ...
                        className="px-6 py-4 bg-red-50 text-red-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all flex items-center gap-2"
                     >
                        <Ban size={14} /> Cancel
                     </button>
                     <button 
                        ...
                        className="px-8 py-4 bg-indigo-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 flex items-center gap-2"
                     >
               ...
               {order.status === 'Approved' && (
                  <button 
                     ...
                     className="px-10 py-5 bg-teal-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-teal-500/20 flex items-center justify-center gap-2"
                  >
