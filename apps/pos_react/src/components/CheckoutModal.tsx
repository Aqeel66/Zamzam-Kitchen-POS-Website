...
export default function CheckoutModal({ 
  ...
  const handleConfirm = () => {
    ...
    if (settings?.branch) {
      ...
      const firstOrderTime = settings.branch.first_order_time ? settings.branch.first_order_time.substring(0, 5) : '00:00';
      const lastOrderTime = settings.branch.last_order_time ? settings.branch.last_order_time.substring(0, 5) : '23:59';
      
      const isOvernight = lastOrderTime < firstOrderTime;
      let isValidTime = true;

      if (isOvernight) {
        isValidTime = currentTime >= firstOrderTime || currentTime <= lastOrderTime;
      } else {
        isValidTime = currentTime >= firstOrderTime && currentTime <= lastOrderTime;
      }

      if (!isValidTime) {
        if (!window.confirm(`WARNING: The current time (${currentTime}) is outside the allowed ordering hours (${firstOrderTime} - ${lastOrderTime}).\n\nDo you still want to proceed?`)) {
          return;
        }
      }
    }
